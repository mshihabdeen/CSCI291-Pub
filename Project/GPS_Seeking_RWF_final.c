...
void move_to_lyt() {
  ...
  bool front_wall = ps_values[0] > 75 || ps_values[7] > 75;
  bool left_wall = ps_values[5] > 100.0 ;
  bool right_wall = ps_values[2]>100;
  
  
  if (left_wall == true){
      set_speed(3.0, 3.0);   // Move forward
  }
  else if (front_wall== true && left_wall == false) {
    set_speed(-1.0, 2.0);  // Turn left
  }
   else if (front_wall== true && left_wall == true) {
       set_speed(2.0, -1.0);  // Turn right
   } 
  else if (front_wall== false && left_wall == false && right_wall == false)
  {
    set_speed(3.0, 3.0);   // Move forward
  }
   else if (front_wall== false && left_wall == true )
  {
    set_speed(2.0, -1.0);  // Turn right
  }
   else if (front_wall== false && left_wall == false)
  {
    set_speed(2.0, -1.0);  // Turn right
  }
}
